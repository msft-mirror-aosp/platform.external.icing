...
class MonkeyAndQueryNodeTest : public ::testing::Test {
 protected:
  void SetUp() override {
    SchemaProto schema =
        SchemaBuilder()
...
TEST_F(MonkeyAndQueryNodeTest, BothSubqueriesReturnEmpty) {
...
TEST_F(MonkeyAndQueryNodeTest, OneSubqueryReturnsEmpty) {
  ...
  std::vector<std::unique_ptr<MonkeyAbstractQueryNode>> children;
  // Subquery 1: "apple" matches {0, 2, 4, 5}.
  children.push_back(std::make_unique<MonkeyTermQueryNode>(
      "apple", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  // Subquery 2: "other" matches {}.
  children.push_back(std::make_unique<MonkeyTermQueryNode>(
      "other", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  MonkeyAndQueryNode node(std::move(children));
  // Intersection of {0, 2, 4, 5} and {} is {}.
  EXPECT_THAT(node.EvaluateQuery(engine_.get()), IsOkAndHolds(IsEmpty()));
}

TEST_F(MonkeyAndQueryNodeTest, NoIntersection) {
  IndexDoc("uri0", {"apple"});                      // doc 0
  IndexDoc("uri1", {"banana"});                     // doc 1
  IndexDoc("uri2", {"apple", "orange"});            // doc 2
  IndexDoc("uri3", {"banana", "grape"});            // doc 3
  ...
  std::vector<std::unique_ptr<MonkeyAbstractQueryNode>> children;
  // Subquery 1: "orange" matches {2, 5}.
  children.push_back(std::make_unique<MonkeyTermQueryNode>(
      "orange", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  // Subquery 2: "grape" matches {3}.
  children.push_back(std::make_unique<MonkeyTermQueryNode>(
      "grape", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  MonkeyAndQueryNode node(std::move(children));
  // Intersection of {2, 5} and {3} is {}.
  EXPECT_THAT(node.EvaluateQuery(engine_.get()), IsOkAndHolds(IsEmpty()));
}

TEST_F(MonkeyAndQueryNodeTest, TwoSubqueriesWithIntersection) {
  IndexDoc("uri0", {"apple"});                      // doc 0
  IndexDoc("uri1", {"banana"});                     // doc 1
  IndexDoc("uri2", {"apple", "orange"});            // doc 2
  IndexDoc("uri3", {"banana", "grape"});            // doc 3
  ...
  children.push_back(std::make_unique<MonkeyTermQueryNode>(
      "apple", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  // Subquery 2: "banana" matches {1, 3, 4, 5}.
  children.push_back(std::make_unique<MonkeyTermQueryNode>(
      "banana", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  MonkeyAndQueryNode node(std::move(children));
  // Intersection of {0, 2, 4, 5} and {1, 3, 4, 5} is {4, 5}.
  EXPECT_THAT(node.EvaluateQuery(engine_.get()),
              IsOkAndHolds(UnorderedElementsAre(4, 5)));
}

TEST_F(MonkeyAndQueryNodeTest, ThreeSubqueriesWithIntersection) {
  IndexDoc("uri0", {"apple"});                      // doc 0
  IndexDoc("uri1", {"banana"});                     // doc 1
  IndexDoc("uri2", {"apple", "orange"});            // doc 2
  IndexDoc("uri3", {"banana", "grape"});            // doc 3
  ...
  children.push_back(std::make_unique<MonkeyTermQueryNode>(
      "banana", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  // Subquery 3: "orange" matches {2, 5}.
  children.push_back(std::make_unique<MonkeyTermQueryNode>(
      "orange", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  MonkeyAndQueryNode node(std::move(children));
  // Intersection of {0, 2, 4, 5}, {1, 3, 4, 5} and {2, 5} is {5}.
  EXPECT_THAT(node.EvaluateQuery(engine_.get()),
              IsOkAndHolds(UnorderedElementsAre(5)));
}

TEST_F(MonkeyAndQueryNodeTest, GenerateQueryString) {
  // 2 children
  ...
  children2.push_back(std::make_unique<MonkeyTermQueryNode>(
      "bar", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  MonkeyAndQueryNode node2(std::move(children2));
  EXPECT_THAT(node2.GenerateQueryString(), Eq("((foo) AND (bar))"));
  ...
  children3.push_back(std::make_unique<MonkeyTermQueryNode>(
      "baz", /*is_prefix=*/false,
      /*is_verbatim=*/false, TermMatchType::EXACT_ONLY));
  MonkeyAndQueryNode node3(std::move(children3));
  EXPECT_THAT(node3.GenerateQueryString(), Eq("((foo) AND (bar) AND (baz))"));
}
