...
libtextclassifier3::Status QualifiedIdJoinIndexImplV3::OptimizeInto(
  ...
  ICING_ASSIGN_OR_RETURN(std::unique_ptr<QualifiedIdJoinIndexImplV3> new_index,
                         Create(filesystem_, new_working_path, feature_flags_));
  ICING_RETURN_IF_ERROR(
      TransferIndex(*document_store, document_id_old_to_new, new_index.get()));
  new_index->set_last_added_document_id(new_last_added_document_id);
  new_index->SetDirty();
